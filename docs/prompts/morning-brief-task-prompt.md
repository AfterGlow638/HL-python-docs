# Korea Semiconductor Morning Brief — Scheduled Task Prompt

> 버전: 2026-07-10  
> 실행 시각: 한국 평일 07:45 KST  
> 용도: ChatGPT Scheduled Task 또는 동등한 반복 브리핑 에이전트

아래 `REFERENCE DOCUMENTS`의 플레이스홀더를 실제 공개 URL 또는 Task가 읽을 수 있는 연결 문서 URL로 바꾼 뒤 사용한다.
...
1. TossInvest WTS API guide:
   {{TOSS_WTS_GUIDE_URL}}
2. Source-tier routing policy:
   {{SOURCE_TIER_GUIDE_URL}}
3. Workflow and interpretation examples:
...
## 10. Streamlit JSON
Return valid JSON only inside this block. Do not include comments or trailing commas.
...
```json
{
  ...
  "data_quality": "",
  "highest_source_tier_required": 1,
  "market_bias": "",
  "confidence": 0.0,
  ...
  "korea_five_session_panel": [
    {
      ...
      "latest_investor_flow": {
        "foreign_net_volume": "",
        "institution_net_volume": "",
        "individual_net_volume": ""
      },
      "five_session_cumulative_flow": {
        "foreign_net_volume": "",
        "institution_net_volume": "",
        "individual_net_volume": ""
      },
      "price_flow_interpretation": "",
      "missing_fields": []
    }
  ],
  "us_five_session_panel": [
    {
      "symbol": "",
  ...
  "korea_market_derivatives": {
    "kospi": "",
    "kosdaq": "",
    "kospi200": "",
    "cash_investor_flow": "",
    "kospi200_futures_flow": "",
    "kospi200_options_flow": "",
    "program_trading": "",
    "nxt": ""
  },
  "binance_overnight": {
    "validated_symbols": [],
    "korean_stock_linked": "",
    "us_semiconductor_linked": "",
    "funding_oi_basis_volume": "",
    "limitations": []
  },
  "cross_asset": {
    "nq_es_sox": "",
...
Task를 만들기 전에 다음 플레이스홀더를 실제 URL로 치환한다.
...
권장 방식은 저장소의 raw Markdown URL 또는 Task가 읽을 수 있는 연결 문서 URL이다. 참조 문서가 프로젝트 내부에만 있고 Task가 해당 파일을 읽지 못하는 환경이라면, 이 프롬프트의 핵심 규칙만으로 실행하고 세부 API 호출은 외부 에이전트가 정규화한 JSON으로 제공한다.
